...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
  ...
  class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
    ...
    public function start_lvl( &$output, $depth = 0, $args = null ) {
      ...
      $classes = array( 'sub-menu tab-content' );
      /**
    ...
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
      ...
      $class_names = $class_names ? ' class="' . esc_attr( $class_names ) .$activeClass. '"' : '';
      ...
      if ( $this->has_children && 0 === $depth ) {
        $fwicon = '<i class="fa fa-chevron-down"></i>';
        ...
        $atts['href'] = ! empty( $item->url ) ? $item->url : 'javscript:void(0)';
        // For items in dropdowns use .dropdown-item instead of .nav-link.
        if ( $depth > 0 ) {
          $atts['class'] = 'dropdown-item';

        } else {
          $atts['class'] = 'xnav-link '.$item->classes[0];
          //var_dump(get_field('menu_typography', $item)["font_family"]);
        }
      }
      ...
      if ( '' !== $linkmod_type ) {
        ...
        $font_style = get_field('menu_typography', $item);
        $font_color = $font_style['text_color'];
        $font_weight = $font_style['font_weight'];
        $font_family =  $font_style["font_family"]  == "initial" ? null :  $font_style["font_family"];
        
        $item_output .= '<a style=\'color: '.$font_color.'; font-weight: '.$font_weight.'; font-family: '.$font_family.'\'' . $attributes . '>';
      }
      ...
      $title = apply_filters( 'the_title', $item->title.' '.$fwicon, $item->ID );
